...
每个高级参数输入框旁有 `❓` 图标，鼠标悬停 0.5 秒弹出气泡显示中文详解
（作用 / 调参建议 / 适用与禁忌场景），见模块常量 `ADV_DESCRIPTIONS`
（dict: 参数名 → 多行说明）。实现为自研 `Tooltip` 类（纯 tkinter：
bind `<Enter>`/`<Leave>` + `widget.after` 延迟 500ms 显示 Toplevel 气泡，
overrideredirect 无边框），无第三方依赖。新增高级参数时需同步补
`ADV_DESCRIPTIONS` 词条。
...
- **v3.1** 8 个高级参数均带 ❓ 且绑定 Enter/Leave 事件 ✅
- **v3.1** Tooltip 气泡显示/销毁逻辑（延迟 500ms 弹出、离开销毁）✅
- **v3.1** 说明内容正确关联参数（lr0 悬停显示学习率介绍）✅
...
- tag 历史：`v1.0`（基础版）→ `v1.1`（环境自动探测）→ `v2.0`（多模型对比/去重/背景图）→ `v3.0`（训练高级参数）→ `v3.1`（高级参数悬停介绍）
